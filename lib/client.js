					...
					(state.plugins.length === 0 ? react.createElement("p", { style: s.empty }, "暂无插件（确认目录下有含 package.json 且非 bundle 的子文件夹）") :
						react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } },
							state.plugins.map(function (p) {
								var badge = p.state === "loaded"
									? { text: p.channel === "loader" ? "已加载·持久" : "已加载·会话", color: "#10b981" }
									: (p.state === "idle" ? { text: "未加载", color: "#3b82f6" }
										: (p.state === "failed" ? { text: "加载失败", color: "#ef4444" }
											: (p.state === "needs-deps" ? { text: "缺依赖声明", color: "#ef4444" }
												: { text: "未构建", color: "#f59e0b" })));
								return react.createElement("div", { key: p.name, style: s.pluginCard },
									...
									p.state === "failed" && p.failReason
										? react.createElement("p", { style: { fontSize: 12, color: "#ef4444", margin: "4px 0 0" } }, "失败原因：" + String(p.failReason).slice(0, 300))
										: null,
