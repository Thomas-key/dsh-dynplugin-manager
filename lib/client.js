...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function InstallDialog(props) {
			...
			var [modePicker, setModePicker] = react.useState(""); // candidate key asking for link/copy choice
			...
			return react.createElement("div", { style: s.overlay, onClick: function (e) { if (e.target === e.currentTarget && !busy) onClose(); } },
				react.createElement("div", { style: s.dialog },
					...
					react.createElement("p", { style: s.hint },
						"安装方式：link＝链接源码目录，改代码重启即生效——要求源码能独立运行、依赖完整（可先在插件目录 npm install）；copy＝独立副本，自动安装依赖（首次较慢），改源码需重新安装。"),
					...
					candidates.length > 0
						? react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } },
							candidates.map(function (cand, i) {
								...
								var canInstall = cand.status === "ready" && !cand.installed;
								var isNpm = cand.sourceKind === "npm";
								var picking = modePicker === key;
								// One install entry: runner → managed; npm → copy;
								// local loader → ask link/copy via an inline picker.
								// Already-installed candidates switch to a mount button.
								var startInstall = function () {
									if (cand.installed) { mount(cand); return; }
									if (isRunner) { install(cand, "managed"); return; }
									if (isNpm) { install(cand, "copy"); return; }
									setModePicker(key);
								};
								var actionLabel = busy === key ? "安装中…"
									: (cand.installed
										? (cand.mounted ? "已加载" : (isRunner ? "已安装" : "挂载"))
										: (isRunner ? "安装到托管目录" : (isNpm ? "安装" : "安装（选方式）")));
								var stateTag = cand.installed
									? (cand.mounted ? { text: "已加载", color: "#10b981" } : { text: "已安装", color: "#3b82f6" })
									: null;
								return react.createElement("div", { key: key, style: s.pluginCard },
									react.createElement("div", { style: s.row },
										...
										stateTag ? react.createElement("span", { style: { fontSize: 12, color: "#fff", background: stateTag.color, borderRadius: 10, padding: "2px 8px", whiteSpace: "nowrap" } }, stateTag.text) : null,
										react.createElement("button", {
											type: "button",
											style: Object.assign({}, s.btnPrimary, canInstall || cand.installed ? {} : s.btnDisabled),
											onClick: startInstall,
											disabled: !!busy || (!canInstall && !cand.installed),
											title: canInstall || cand.installed ? "" : (cand.status === "needs-deps" ? "需先在 package.json 声明依赖：" + (cand.imports || []).join(", ") : "入口文件缺失，请先构建"),
										},
											actionLabel)
									),
									cand.description ? react.createElement("p", { style: s.pluginDesc }, String(cand.description)) : null,
									cand.status !== "ready"
										? react.createElement("p", { style: s.error },
											cand.status === "needs-deps"
												? "缺依赖声明：" + (cand.imports || []).join(", ") + " — 在 package.json 补声明后重扫"
												: "未构建：入口文件缺失，请先构建（如 tsc 编译）")
										: null,
									picking
										? react.createElement("div", { style: Object.assign({}, s.row, { background: "var(--dsw-alias-bg-layer-2)", border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 8, padding: "8px 10px" }) },
											react.createElement("span", { style: { fontSize: 12, color: "var(--dsw-alias-label-secondary)" } }, "安装方式："),
											react.createElement("button", { type: "button", style: s.btn, onClick: function () { setModePicker(""); install(cand, "link"); }, disabled: !!busy }, "link"),
											react.createElement("button", { type: "button", style: s.btn, onClick: function () { setModePicker(""); install(cand, "copy"); }, disabled: !!busy }, "copy"),
											react.createElement("button", { type: "button", style: s.btnDanger, onClick: function () { setModePicker(""); }, disabled: !!busy }, "取消")
										)
										: null,
		...
		function DynPluginManager() {
			...
			var act = function (url, name, label) {
				setBusy(name + ":" + label);
				setNotice("");
				fetch(url, {
					method: "POST",
					...
					.then(function (r) { return r.json(); })
					.then(function (p) {
						if (!p || p.ok !== true) throw new Error((p && p.text) || (p && p.error) || (label + " failed"));
						setNotice(String(p.text || (label + " 完成")));
						refresh();
					})
			...
			return react.createElement("div", { style: s.wrap },
				...
				react.createElement("p", { style: s.hint },
					"列表来自托管目录(runner 自包含插件) + profile 已安装的非 bundle 包。加载在会话中用 /dynload <插件名>（自包含→会话级，需 import→持久挂载）；loader 插件可在下方停用/卸载；runner 保持会话级，请在会话中加载与停止。"),
				state.error ? react.createElement("p", { style: s.error }, String(state.error)) : null,
				notice ? react.createElement("p", { style: notice.startsWith("✗") ? s.error : s.ok, whiteSpace: "pre-wrap" }, String(notice)) : null,
									...
									react.createElement("div", { style: s.row },
										...
										isLoader
											? react.createElement("button", {
												type: "button",
												style: s.btn,
												onClick: function () { if (!busy) act("/api/dynplugin/unmount", p.name, "停用"); },
												disabled: !!busy || p.state !== "loaded",
											}, busy === p.name + ":停用" ? "停用中…" : "停用")
											: null,
										isLoader
											? react.createElement("button", {
												type: "button",
												style: s.btnDanger,
												onClick: function () { if (!busy) act("/api/dynplugin/uninstall", p.name, "卸载"); },
												disabled: !!busy,
											}, busy === p.name + ":卸载" ? "卸载中…" : "卸载")
											: null
									),
