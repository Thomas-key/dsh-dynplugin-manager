...
export function apply(ctx) {
  ...
  /**
   * Unmount one plugin: loader → remove managed insert row (live unmount);
   * runner → delete its managed-dir copy. Shared by /dynunmount and the UI.
   */
  async function unmountPlugin(target) {
    ...
    try {
      ...
      if (target.channel === 'runner') {
        if (isUnder(target.dir, managedDir())) {
          ...
          return { kind: 'success', text: `Removed ${pkgName} from the managed dir (${target.dir}). Reinstall it through the dialog if needed.` }
        }
        return { kind: 'error', text: `"${pkgName}" is a runner plugin but not installed in the managed dir (${target.dir}) — there is no persistent trace to remove.` }
      }
  ...
  /**
   * Fully remove one plugin: unmount + remove the package (pnpm) + managed
   * copy + failure records. Shared by /dynuninstall and the UI.
   */
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const target = plugins.find((p) => p.name === name)
          if (!target) { json(res, 404, { ok: false, error: `Plugin "${name}" not found` }); return }
          const result = await unmountPlugin(target)
          json(res, result.kind === 'success' ? 200 : 400, { ok: result.kind === 'success', text: result.text })
        } catch (error) {
          json(res, 500, { ok: false, error: error instanceof Error ? error.message : String(error) })
        }
      },
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const target = plugins.find((p) => p.name === name)
          if (!target) { json(res, 404, { ok: false, error: `Plugin "${name}" not found` }); return }
          const result = await uninstallPlugin(target)
          json(res, result.kind === 'success' ? 200 : 400, { ok: result.kind === 'success', text: result.text })
        } catch (error) {
          json(res, 500, { ok: false, error: error instanceof Error ? error.message : String(error) })
        }
