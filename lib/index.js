...
//
// Two load channels:
//   - runner channel (/dynload): code is handed to dynamicCordisRunner as
//     text; its vm sandbox forbids module loading, so only self-contained
//     plugins (no import/require) work here.
//   - loader channel (/dynmount): community npm plugins (real imports) are
//     mounted via a managed insert row in the profile's cordis.patch.yml;
//     the official patch watcher applies it live (no restart, no HMR code).
//
...
export function apply(ctx) {
  ...
  // ── slash command /dynload <name> (runner channel, self-contained plugins)
  ctx.effect(() => commands.register({
    name: 'dynload',
    description: 'Load a self-contained dynamic plugin by its scanned display name (runner channel; see Settings → 动态插件).',
    input: { hint: '<插件名>' },
    handler: async (invocation) => {
      ...
      if (target.status !== 'ready') {
        return { kind: 'error', text: `Plugin "${raw}" has no built entry file (${target.dir}); build it first or fix package.json main.` }
      }
      if (target.channel === 'loader') {
        return { kind: 'error', text: `Plugin "${raw}" uses import/require — the runner sandbox cannot host it. Use /dynmount ${raw} (loader channel, persistent).` }
      }
      try {
  ...
  // ── slash command /dynmount <name> (loader channel, community npm plugins)
  // Mounts an installed package via a managed insert row in the profile's
  // cordis.patch.yml; the OFFICIAL patch watcher applies it live (no restart).
  ctx.effect(() => commands.register({
    name: 'dynmount',
    description: 'Mount a community npm plugin (uses import) via insert row + official patch watcher. Persistent across restarts.',
    input: { hint: '<插件名>' },
    handler: async (invocation) => {
      const raw = invocation.rawInput.trim()
      if (!raw) {
        return { kind: 'error', text: 'Usage: /dynmount <plugin-name>. The plugin must be installed into the profile first (dsh plugin add -w <dir> or dsh plugin add <pkg>).' }
      }
      const { plugins } = scanAll(readDirs())
      const target = plugins.find((p) => p.name === raw)
      if (!target) {
      ...
      if (target.status !== 'ready') {
        return { kind: 'error', text: `Plugin "${raw}" has no built entry file (${target.dir}); build it first.` }
      }
      const profile = currentProfile()
      if (!isInstalled(profile, target.rawName)) {
        return {
          kind: 'error',
          text: `Package "${target.rawName}" is not installed in profile "${profile}". Install it first, e.g. 'dsh plugin --profile ${profile} add -w ${target.dir}' (or add <pkg> from npm), then /dynmount ${raw}.`,
        }
      }
      try {
        ...
        return {
          kind: 'success',
          text: `Mounted ${target.rawName} into profile "${profile}" (${state}). It persists across restarts; unmount with /dynunmount ${raw}.`,
        }
      } catch (error) {
    ...
    for (const plugin of plugins) {
      ...
      const disposer = commands.register({
        ...
        handler: async (invocation) => {
          ...
          if (target.status !== 'ready') {
            return { kind: 'error', text: `Plugin "${pluginName}" has no built entry file; build it first.` }
          }
          if (target.channel === 'loader') {
            return { kind: 'error', text: `Plugin "${pluginName}" uses import/require — use /dynmount ${pluginName} instead (loader channel, persistent).` }
          }
          try {
            const def = await loadPlugin(invocation.agent, runner, target, invocation.signal)
            return {
