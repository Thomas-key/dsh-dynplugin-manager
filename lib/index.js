...
import { readFileSync, writeFileSync, renameSync, mkdirSync, readdirSync, statSync, rmSync, cpSync } from 'node:fs'
import { join, dirname, basename, resolve } from 'node:path'
import { homedir } from 'node:os'
import { execFile } from 'node:child_process'
import { dshHome, currentProfile, writeInsertRow, removeInsertRow, hasManagedRow, isInstalled, needsLoaderChannel, extractImports } from './loader.js'
...
/**
 * Scan every source: user-configured dirs (kept for backward compat) +
 * the runner managed dir + the profile's node_modules (loader plugins
 * installed via dsh add). Returns { plugins, dirs }.
 */
export function scanAll(dirs) {
  ...
  if (!seenDirs.has(managed)) {
    const found = scanDir(managed, taken)
    plugins.push(...found)
    dirResults.push({ path: managed, alias: '托管目录(runner)', count: found.length })
    seenDirs.add(managed)
  }
  // Built-in source 2: profile node_modules (loader plugins from dsh add).
  const nmPlugins = scanNodeModules(currentProfile(), taken)
  plugins.push(...nmPlugins)
  if (nmPlugins.length > 0) {
    dirResults.push({ path: join(profileDirOf(currentProfile()), 'node_modules'), alias: '已安装(loader)', count: nmPlugins.length })
  }
...
export function apply(ctx) {
  ...
  async function scanInstallSource(source, kind) {
    if (kind === 'npm') {
      ...
      return {
        candidates: [{
          name: String(meta.name || source),
          rawName: String(meta.name || source),
          dir: null,
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (!isInstalled(profile, target.rawName, target.dir)) {
            json(res, 400, { ok: false, error: `Package "${target.rawName}" is not installed in profile "${profile}".` })
            return
          }
          const rowId = slugify(target.rawName)
          if (hasManagedRow(profile, rowId)) {
