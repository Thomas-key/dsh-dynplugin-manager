...
import { currentProfile, writeInsertRow, removeInsertRow, hasManagedRow, isInstalled, needsLoaderChannel, extractImports } from './loader.js'
  ...
  async function handleLoad(ctx, agent, target, signal) {
    ...
    if (target.channel === 'loader') {
      ...
      try {
        ...
        // resolution + real apply(ctx) run here — if the plugin cannot load,
        // nothing was written and there is nothing to roll back.
        const live = await liveApplyInsert(ctx, profile, rowId, target.rawName)
        if (!live.ok && IMPORT_FAILURE.test(live.message ?? '')) {
          return {
            kind: 'error',
            text: `Mount of ${target.rawName} FAILED: ${live.message}\nNothing was written to the profile — retry after fixing the plugin's dependencies.`,
          }
        }
        ...
        const state = live.ok ? 'applied live' : `file written (${live.message}); it will apply on next restart`
        return {
          kind: 'success',
