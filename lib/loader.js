...
/**
 * Whether a package is resolvable on the loader channel's REAL resolution
 * chain. The loader imports a mounted package from its file's real location,
 * walking UP node_modules directories — the shared profiles/node_modules
 * fallback is NOT on that chain (it serves the bundle layer stack only; bare
 * imports fail from a plugin's real location, verified 2026-08-15):
 *   - npm-installed packages live under profile/node_modules/.pnpm → their
 *     chain covers profile node_modules;
 *   - link-installed packages live at their source path → their chain covers
 *     the plugin directory's own node_modules (a local `npm install` there).
 */
