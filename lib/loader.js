...
/** Whether the profile's patch already contains a managed block for rowId. */
export function hasManagedRow(profile, rowId) {
  try {
    return hasManagedBlock(readFileSync(patchPath(profile), 'utf8'), rowId)
  } catch {
    return false
  }
}
...
/**
 * Whether a package resolves with Node's REAL resolution algorithm from a
 * plugin directory — the exact chain the loader's import uses (walking UP
 * node_modules from the file's real location; pnpm .pnpm virtual stores are
 * covered because a mounted package's real path sits inside one). This is
 * the precise quality-gate check: directory-existence tests misjudge pnpm
 * layouts (deps live in .pnpm, not the top level).
 */
export function resolvesFrom(dir, spec) {
  try {
    createRequire(join(dir, '__probe__.js')).resolve(spec)
    return true
  } catch {
    return false
  }
...
/**
 * Whether a package is INSTALLED (directory-level): profile node_modules or
 * the plugin's own node_modules. Used for "is the plugin itself added" —
 * dependency resolution uses resolvesFrom instead.
 */
export function isInstalled(profile, pkgName, extraRoot = null) {
  const roots = [join(profileDir(profile), 'node_modules')]
  if (extraRoot !== null && typeof extraRoot === 'string' && extraRoot.length > 0) {
    roots.push(join(extraRoot, 'node_modules'))
  }
...
/**
 * Statically extract BARE module specifiers from code: the packages the code
 * imports/requires, minus `node:` builtins and relative/absolute paths.
 * Used by the runner-channel quality gate — if the package.json declares no
 * dependencies yet the code imports bare specifiers, the user must declare
 * them. Coarse parse: false positives are just extra names in the hint.
 */
export function extractImports(code) {
  if (typeof code !== 'string' || code.length === 0) return []
  return collectSpecifiers(code)
}

function collectSpecifiers(code) {
  const found = new Set()
  const patterns = [
    /\bimport\s*\(?\s*['"]([^'"]+)['"]/g,          // import 'x' / import('x')
  ...
  return [...found].filter((s) => !isRelativeOrBuiltin(s))
}

function isRelativeOrBuiltin(spec) {
