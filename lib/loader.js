 ...
 * a top-level `import`/`require` statement means the runner sandbox (which
 * forbids module loading) cannot host it.
 */
export function needsLoaderChannel(code) {
  if (typeof code !== 'string') return false
  // import statements/expressions are anchored at line start; `require(` may
  // appear mid-line (`const x = require(...)`). Coarse sniff: false positives
  // just route the user to the loader channel, which is harmless.
  return /^\s*import[\s(]|^\s*export\s+[^;]*\s+from\s+|require\s*\(/m.test(code)
}
